  ...
  <view
    ...
    <view class="le-dropdown-menu">
      <view class="le-dropdown-menu-item" v-for="(item, index) in menuList" :key="index" @tap.stop="menuClick(index)">
        <view class="le-flex">
          <text class="le-dropdown-menu-item-text" :style="{ color: index === current ? themeColor : inactiveColor }">{{ item.title }}</text>
          <view
            v-if="item.type === 'sort'"
            :class="[
          ...
          <view class="le-dropdown-menu-item-basicarrow" v-else-if="item.type === 'time'"></view>
          <view v-else :class="['le-dropdown-menu-item-basicarrow', index === current && 'le-dropdown-menu-item-basicarrow_rotate']"></view>
        </view>
      </view>
    </view>
    <!-- 展示的内容 -->
    <view
      ...
      <view ref="leDropdownContentPopupRef" class="le-dropdown-content-popup" :style="[popupStyle]" @tap.stop.prevent>
        <block v-for="(item, index) in menuList" :key="index">
          <view class="le-dropdown-popup-content le-dropdown-cell" v-if="item.type === 'time' && index === current">12456</view>
          <!-- 单选列表 -->
          ...
          <view class="le-dropdown-popup-content le-dropdown-filter" v-if="item.type === 'filter' && index === current">
            <view class="le-dropdown-filter-item" v-for="(sItem, sIndex) in item.children" :key="sIndex">
              <view class="le-dropdown-filter-title">
                {{ sItem.title }}
                <text class="le-dropdown-filter-subtitle" v-if="sItem.type === 'slider'">{{ sItem.value }}{{ sItem.suffix }}</text>
              </view>
...
const menuClick = (index) => {
  // console.log(props.menuList[index].type)
  // console.log(props.menuList[index])
  getContentHeight()
...
// 时间
const onConfirmDateTime = (e) => {
  // console.log(timeObj)
  timeObj.value = dayjs(e.value).format('YYYY-MM-DD HH:mm')
  showDatetime.value = false
  emits('onConfirm', timeObj)
}
// 单选列表选中事件
...
const resetMenuCellList = (menuList, filterList) => {
  // console.log('333', menuList, filterList)

  menuList.forEach((item) => {
    if (item.type !== 'filter') {
      // console.log(item)
      filterList.children.forEach((jItem) => {
        if (item.props === jItem.props) {
          // console.log(jItem.options.filter((oItem) => oItem.value === jItem.value)[0].label)

          jItem.options.forEach((oItem) => {
            if (oItem.value === jItem.value) {
              item.title = oItem.label
            }
...
const onFilterConfirm = (item, index) => {
  close()
  resetMenuCellList(props.menuList, item)
